...
export async function PUT(req: NextRequest) {
  try {
    const body = await req.json();
    const { gameId, action, userId, userName, ...actionData } = body;
...
async function handleSubmitWord(gameId: string, userId: string, actionData: any) {
  ...
  // Validate word first
  const validationResponse = await fetch(`${process.env.NEXT_PUBLIC_URL}/api/wordchain/validate`, {
    method: 'POST',
